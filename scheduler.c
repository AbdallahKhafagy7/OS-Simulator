...
void printLog(PCB* p, char *string) {
    ...
    if (strcmp(string, "finished") == 0) {
        ...
        fprintf(pFile,
                ...
                getClk(), p->process_id, string, p->arrival_time,
                p->RUNNING_TIME, p->REMAINING_TIME, W, TA, WTA);
    } else {
        fprintf(pFile,
                "At time %d process %d %s arrival %d total %d remain %d\n",
                getClk(), p->process_id, string, p->arrival_time,
                p->RUNNING_TIME, p->REMAINING_TIME);
    }
    fclose(pFile);
}
    ...
    while(1)
    {
        ...
        if(rec_status!=-1)
        {
            switch(selected_Algorithm_NUM) {
                ...
                case 2: {
                    // SRTN - Keep your existing SRTN code
                ...
                case 3: {
                    ...
                    if(running_process_index == -1 && peek_front(&READY_QUEUE) != NULL && 
                       peek_front(&READY_QUEUE)->Process.first_time) {
                        ...
                        pcb[process_count].START_TIME = -1;  // Will be set when actually runs
                        pcb[process_count].LAST_EXECUTED_TIME = -1;
                        ...
                        pcb[process_count].num_pages = (peek_front(&READY_QUEUE)->Process.limit + PAGE_SIZE - 1) / PAGE_SIZE;
                        
                        // Copy memory requests
                        pcb[process_count].num_requests = peek_front(&READY_QUEUE)->Process.num_requests;
                        ...
                        int result = init_process_page_table(&pcb[process_count]);
                        if(result == -1) {
        ...
        if (selected_Algorithm_NUM == 2 && clock_timer != getClk()) {
            ...
            if (running_process_index != -1 && front_index != -1 && 
                pcb[front_index].REMAINING_TIME < pcb[running_process_index].REMAINING_TIME) {
                kill(pcb[running_process_index].process_pid, SIGSTOP);
                pcb[running_process_index].process_state = Ready;
            ...
            if (running_process_index != -1) {
                if (pcb[running_process_index].REMAINING_TIME <= 0) {
                   ...
                   WTA[count] = (float)(pcb[running_process_index].FINISH_TIME - pcb[running_process_index].arrival_time) / 
                                pcb[running_process_index].RUNNING_TIME;
                    ...
                    if (peek_priority_front(&READY_PRIORITY_QUEUE) != NULL) {
                        ...
                        if (running_process_index != -1 && pcb[running_process_index].process_state == Ready &&
                            peek_priority_front(&READY_PRIORITY_QUEUE)->first_time!=true) {

                            kill(pcb[running_process_index].process_pid, SIGCONT);
                            pcb[running_process_index].process_state = Running;
                            pcb[running_process_index].LAST_EXECUTED_TIME = getClk();
                            pcb[running_process_index].WAITING_TIME = getClk() - pcb[running_process_index].arrival_time -
                                                                      (pcb[running_process_index].RUNNING_TIME - 
                                                                       pcb[running_process_index].REMAINING_TIME);
        ...
        if(selected_Algorithm_NUM == 3 && clock_timer != getClk()) {
            ...
            while(temp_process != NULL) {
                ...
                if(index != -1 && pcb[index].process_state == Blocked) {
                    ...
                    if(pcb[index].blocked_time <= 0) {
                        ...
                        printf("Process %d unblocked at time %d\n", pcb[index].process_id, getClk());
            ...
            // Handle running process memory requests
            if(running_process_index != -1 && pcb[running_process_index].process_state == Running) { 
                
                if(pcb[running_process_index].num_requests > 0) {
                    request* current_request = &pcb[running_process_index].memory_requests[0];
                    int elapsed_time = getClk() - pcb[running_process_index].START_TIME;
                    
                    if(current_request->time <= elapsed_time) {
                        int virtual_page = get_vpn(current_request->address);
                        
                        printf("Process %d requesting page %d at time %d (address=%d, rw=%c)\n",
                               pcb[running_process_index].process_id, virtual_page, 
                               getClk(), current_request->address, current_request->rw);
            ...
            if(running_process_index != -1 && 
               ...
               pcb[running_process_index].process_state == Ready) {
                ...
                pcb[running_process_index].START_TIME = getClk();
                
                printf("Process %d starting for first time at %d, loading page 0\n", 
                       pcb[running_process_index].process_id, getClk());
                
                int page_fault = Request(pcb, process_count, 
                                        pcb[running_process_index].process_id, 0, 'r');
                ...
                if(page_fault == 1) {
                    ...
                    printf("Process %d blocked for initial page load\n", 
                           pcb[running_process_index].process_id);
                    
                    pFile = fopen("scheduler.log", "a");
                    if(pFile) {
                        fprintf(pFile, "At time %-5d process %-5d blocked (initial) arr %-5d total %-5d remain %-5d wait %-5d\n",
                                getClk(), pcb[running_process_index].process_id,
                                pcb[running_process_index].arrival_time,
                                pcb[running_process_index].RUNNING_TIME,
                                pcb[running_process_index].REMAINING_TIME,
                ...
                char str_rem_time[20];
                sprintf(str_rem_time, "%d", pcb[running_process_index].RUNNING_TIME);
            ...
            if(running_process_index != -1 && 
               ...
               pcb[running_process_index].REMAINING_TIME <= 0) {
                ...
                free_process_pages(pcb[running_process_index].process_id, 
                                  &pcb[running_process_index]);
                printf("Freed all pages for process %d\n", pcb[running_process_index].process_id);
                ...
                if(peek_front(&READY_QUEUE) != NULL) {
                    ...
                    if(running_process_index != -1) {
                        if(pcb[running_process_index].START_TIME == -1) {
                            // First time - need to load page 0
                            pcb[running_process_index].START_TIME = getClk();
                        ...
                        else if(pcb[running_process_index].process_state == Ready) {
                            // Resume
                            kill(pcb[running_process_index].process_pid, SIGCONT);
                            pcb[running_process_index].process_state = Running;
                            pcb[running_process_index].LAST_EXECUTED_TIME = getClk();
            ...
            if(running_process_index != -1 && 
               ...
               pcb[running_process_index].REMAINING_TIME > 0) {
                ...
                if(time_executed >= TIME_QUANTUM) {
                    ...
                    process_Node* temp_node = dequeue(&READY_QUEUE);
                    enqueue(&READY_QUEUE, temp_node->Process);
